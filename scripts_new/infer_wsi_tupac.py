...
def parse_args(argv=None):
    ...
    p.add_argument("--fixed-thresh", type=float, default=None,
                   help="TUPAC / cross-dataset mode: skip the val sweep and "
                        "apply THIS det_thresh directly to both val and test "
                        "(e.g. a threshold carried over from MIDOG++). When "
                        "set, --thr-min/--thr-max/--thr-step are ignored.")
...
def load_split_manifest(arg, allow_overlap=False):
    """Return (val_ids, test_ids) sets from a split manifest.

    allow_overlap: in fixed-threshold mode no tuning happens on val, so val and
    test may be identical (e.g. all slides in both) without any leak. Callers
    pass allow_overlap=True then.
    """
    path = Path(arg)
    ...
    if overlap and allow_overlap:
        log(f"Split manifest: {len(val)} val / {len(test)} test slide(s); "
            f"{len(overlap)} overlap ALLOWED (fixed-threshold mode, no tuning).")
    else:
...
def write_split_report(out_dir, name, det_arrays, gt_points, slides,
    ...
    if False and len(per_tumor) > 1:   # TUPAC: single domain -> overall only
        log("-- per tumour type --")
...
def main(argv=None):
    ...
    if not 0.0 <= args.overlap < 1.0:
        ...
        return 1
    if args.fixed_thresh is None:
        if not (args.thr_min < args.thr_max and args.thr_step > 0):
            log("ERROR: need --thr-min < --thr-max and --thr-step > 0.")
            return 1
    else:
        if not (0.0 <= args.fixed_thresh < 1.0):
            log("ERROR: --fixed-thresh must be in [0, 1).")
            return 1
    ...
    val_slides, test_slides = load_split_manifest(
        args.slides, allow_overlap=(args.fixed_thresh is not None))
    ...
    if args.fixed_thresh is not None:
        # TUPAC / cross-dataset: do NOT tune on this dataset. Apply the supplied
        # threshold (carried over from MIDOG++ val) directly. The "grid" is a
        # single point so the FROC code (which sweeps thr_grid) still has a
        # sensible neighbourhood; we report at exactly fixed_thresh.
        best_thr = round(float(args.fixed_thresh), 6)
        # build a small grid around it ONLY for the FROC curve, clipped to (0,1)
        thr_grid = sorted({round(max(1e-6, min(0.999999, best_thr + d * args.thr_step)), 6)
                           for d in range(-50, 51)})
        thr_grid = [t for t in thr_grid if t >= args.score_floor]
        if best_thr not in thr_grid:
            thr_grid = sorted(set(thr_grid) | {best_thr})
        # one sweep pass so `sweep` (val curve at each grid pt) is still populated
        _, sweep = optimise_threshold(
            det_arrays, gt_points, val_slides, thr_grid, args.radius)
        log(f"\nFIXED threshold mode: det_thresh = {best_thr:g} "
            f"(supplied via --fixed-thresh; NO tuning on this dataset).")
    else:
    ...
    if args.fixed_thresh is not None:
        log(f"\nThreshold {best_thr:g} was SUPPLIED (--fixed-thresh, carried "
            f"over from another dataset) and applied unchanged to val and test. "
            f"No tuning was performed on this dataset. Test numbers are the "
            f"ones to report.")
    else:
