# scripts/train_deformable_detr_r50frozen_midogpp.py
"""Training script for the FROZEN ResNet-50 deformable_detr_r50frozen baseline on MIDOG++.

Loads the config, verifies patient-stratified splits and an EarlyStoppingHook,
then runs the MMDetection runner. The ResNet-50 backbone is frozen (see config:
frozen_stages=4, norm_eval=True); only the neck + detection head are trained.
"""
import json
import sys
...
CONFIG_NAME = "deformable_detr_r50frozen_midogpp.py"
