# configs/retinanet_uni2h_midogpp.py
#
# RetinaNet with a FROZEN UNI2-h (custom ViT-H/14) backbone +
# SimpleFeaturePyramid neck, for mitotic-figure detection on MIDOG++.
#
# Aligned to faster_rcnn_uni2h_midogpp.py / retinanet_uni_midogpp.py so the
# RetinaNet row differs ONLY in the detector head. Augmentation and RetinaNet
# head hyperparameters are held constant with the UNI RetinaNet config.
#
# BACKBONE (UNI2-h model card, MahmoodLab/UNI2-h):
#   - custom ViT-H/14: patch 14, embed_dim 1536, depth 24, SwiGLU, 8 reg tokens.
#   - ImageNet normalization (DINOv2 recipe).
#
# *** PATCH 14 -> 1008 input, patches_1008, strides [7,14,28,56,112] ***
#   RetinaNet is anchor-based, so (unlike DETR) the AnchorGenerator strides
#   MUST match the patch-14 token map. Using UNI's patch-16 [8,16,32,64,128]
#   here would misalign every anchor by ~14%. This mirrors the patch-14 stride
#   handling in faster_rcnn_uni2h_midogpp.py.
...
model = dict(
    ...
    bbox_head=dict(
        ...
        anchor_generator=dict(
            ...
            scales_per_octave=3,
            ratios=[0.5, 1.0, 2.0],
            strides=[7, 14, 28, 56, 112],   # patch-14 -> correct for UNI2-h
            center_offset=0.5,
        ),
        bbox_coder=dict(
...
resume = False
work_dir = './outputs/work_dirs/retinanet_uni2h_1024_100epochs'
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='COMPAYL26',
            name='retinanet_uni2h_midogpp',
            group='retinanet',
            tags=['UNI2-h', 'retinanet', 'midogpp', 'frozen', 'augmented'],
        ),
    ),
]
visualizer = dict(
