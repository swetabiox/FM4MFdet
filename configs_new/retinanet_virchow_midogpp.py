...
#
# RetinaNet with a FROZEN Virchow (ViT-H/14) backbone + SimpleFeaturePyramid
# neck, for mitotic-figure detection on MIDOG++.
#
# Aligned to the UNI / UNI2-h RetinaNet configs so the RetinaNet row differs
# ONLY in the detector head. Augmentation and RetinaNet head hyperparameters
# are held constant across the row.
#
# BACKBONE (Virchow model card, paige-ai/Virchow):
#   - ViT-H/14: patch 14, embed_dim 1280, 32 layers, SwiGLU, CLS token only
#     (no register tokens).
...
#   patch-14 token map (same as UNI2-h; differs from UNI's patch-16 scheme).

custom_imports = dict(
    imports=[
        'src.custom_mmdet.backbones.virchow_vit',
...
model = dict(
    ...
    data_preprocessor=dict(
        ...
        mean=[123.675, 116.28, 103.53],   # ImageNet RGB mean (Virchow / DINOv2)
    ...
    neck=dict(
        ...
        in_channels=1280,                 # Virchow embed_dim (ViT-H)
        out_channels=256,
    ...
    bbox_head=dict(
        ...
        anchor_generator=dict(
            ...
            scales_per_octave=3,
            ratios=[0.5, 1.0, 2.0],
            strides=[7, 14, 28, 56, 112],   # patch-14 -> correct for Virchow
            center_offset=0.5,
        ),
        bbox_coder=dict(
...
resume = False
work_dir = './outputs/work_dirs/retinanet_virchow_1024_100epochs'
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='COMPAYL26',
            name='retinanet_virchow_midogpp',
            group='retinanet',
            tags=['Virchow', 'retinanet', 'midogpp', 'frozen', 'augmented'],
        ),
    ),
]
visualizer = dict(
